...
class collect_data:
    def __init__(self, start_date='2012-06-01', end_date='2022-06-01'):
        ...
        self.ftse100_companies = pd.read_excel('data/ftse100_companies.xlsx')
        self.ftse100_tickers = self.ftse100_companies['Code'].tolist()
        for i in range(len(self.ftse100_tickers)):
            if (self.ftse100_tickers[i])[-1] == '.':
                self.ftse100_tickers[i] = self.ftse100_tickers[i] + 'L'
            else:
                self.ftse100_tickers[i] = self.ftse100_tickers[i] + '.L'
    ...
    def download_ftse100(self):
        for i in range(len(self.ftse100_tickers)):
            ...
            df = yf.download(self.ftse100_tickers[i], self.start_date, self.end_date)
            df.to_csv('data/ftse100/' + self.ftse100_tickers[i] + '.csv')
